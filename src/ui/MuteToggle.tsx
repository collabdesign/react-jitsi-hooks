...
type MuteToggleProps = {
  ...
  unMutedElement: React.ReactNode
...
const MuteToggle = ({
  ...
  const track = localTracks.find(
    (track) => !track.jitsi.isEnded() && track.type === 'audio'
  )
  const availableTracks = localTracks.filter(
    (track) => !track.jitsi.isEnded() && track.type === 'audio'
  )
  ...
  if (track?.jitsi.isMuted()) {
    return (
      <a onClick={onClick} href="#unmute">
        {track.jitsi.getDeviceId()}
        {mutedElement}
      </a>
    )
  }
  return (
    <a onClick={onClick} href="#mute">
      {unMutedElement}
    </a>
  )
}
